...
class ProductRepository implements ProductRepositoryInterface
{
    ...
    public function getByCategory($input)
    {
        ...
        switch ($order) {
            case 'news':
                return Product::filter($input)->with($product_entities)->select('product.*','product_translation.*')->paginate($number_per_page);
                break;
            case 'low_price_to_high':
                return Product::filter($input)->with($product_entities)->select('product.*','product_translation.*')->orderBy('best_price','asc')->paginate($number_per_page);
                break;
            case 'high_price_to_low':
                return Product::filter($input)->with($product_entities)->select('product.*','product_translation.*')->orderBy('best_price','desc')->paginate($number_per_page);
                break;
            case 'best_rating':
                return Product::filter($input)->with($product_entities)->select('product.*','product_translation.*')->orderBy('rating','desc')->paginate($number_per_page);
                break;
            case 'discount':
                return Product::filter($input)->with($product_entities)->select('product.*','product_translation.*')->orderByRaw("(((original_price - best_price) * 100) /original_price) DESC")->paginate($number_per_page);
                break;
            case 'brand_a_z':
                return Product::filter($input)->with($product_entities)->select('product.*','product_translation.*')->orderBy('order_brand_by','asc')->paginate($number_per_page);
                break;
            case 'brand_z_a':
                return Product::filter($input)->with($product_entities)->select('product.*','product_translation.*')->orderBy('order_brand_by','desc')->paginate($number_per_page);
                break;
            default:
                break;
    ...
    //utiliser pour avoir les produits des class_parents(class)
      ...
      public function getProductByCategory($input)
    {
        $product_entities = [
            "images",
            "url",
        ...
        return Product::filter($input)->with($product_entities)->select('product.*','product_translation.*')->get();
